...
def fetch_tax_amount(row,inv):
	...
	total_tax = 0
	if frappe.get_value("POS Invoice",inv,'taxes_and_charges'):
		tax_details = frappe.get_all("Sales Taxes and Charges",\
			{"parent":frappe.get_value("POS Invoice",inv,'taxes_and_charges'),'charge_type':"On Net Total"},['rate'])
		if tax_details:
...
def invoice_details(inv,filters):
	...
	item_deets = frappe.get_all("POS Invoice Item",filter_dict,['item_code',\
		'item_name','item_group','cost_center','stock_uom','rate','amount','item_tax_template',\
		'base_rate','base_amount','stock_qty'])
	sales_inv_deets = frappe.get_all("POS Invoice",{'name':inv},['territory','customer_name','owner','posting_date','currency'])
...
def fetch_data(filters):
	...
	all_inv = frappe.get_all("POS Invoice",filter_dict)
	for i in all_inv:
