...
| **B-13** | Pricing Policy Change Audit Notification — zero-code Frappe Notification on `Sungas Pricing Policy` Value-Change event that emails CFO + Head of Finance whenever any threshold (flag %, block %, margin band, SLA mins, etc.) is changed. Reuses Frappe's built-in Version tracking — pure config in Desk, no fixture. Gives executive visibility on threshold tampering. | 🟢 P3 |
